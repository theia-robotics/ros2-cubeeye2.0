...
public:
	...
	virtual Type _decl_call type(int8u idx = 0) = 0;
...
public:
	...
	virtual result _decl_call run(int32s wantedFrame = FrameType::Depth | FrameType::Amplitude) = 0;
...
using CameraState = CubeEyeCamera::State;
using CameraType = CubeEyeCamera::Type;
using CameraError = CubeEyeCamera::Error;
...
_decl_dll result _decl_call set_properties(const sptr_properties& properties);
_decl_dll result_property _decl_call get_property(const std::string& key);
_decl_dll result_properties _decl_call get_properties(const std::string& name);
